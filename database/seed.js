...
router.get("/seed", async (req, res) => {
  try {
    ...
    await pool.query(`
      ...
      VALUES ('nirakar', 'nirakar@example.com', '$2a$10$zTfVba8KozFqKUH0NT5K0ONuOkxQ3/0kKmvXdf82PpJFr4Q0Srq7a')
    ...
    await pool.query(`
      INSERT INTO login_information (key, language, value) VALUES
      ('login_title', 'en', 'Login to your account'),
      ('login_title', 'se', 'Logga in på ditt konto'),
      ('email_placeholder', 'en', 'Enter your email'),
      ('email_placeholder', 'se', 'Ange din e-post'),
      ('password_placeholder', 'en', 'Enter your password'),
      ('password_placeholder', 'se', 'Ange ditt lösenord'),
      ('login_button', 'en', 'Login'),
      ('login_button', 'se', 'Logga in')
      ON CONFLICT DO NOTHING;
    `);

    await pool.query(`
      CREATE TABLE IF NOT EXISTS pricelist_headers (
        id SERIAL PRIMARY KEY,
        key VARCHAR(100) UNIQUE,
        label_en VARCHAR(255),
        label_se VARCHAR(255)
      );
    `);

    await pool.query(`
      INSERT INTO pricelist_headers (key, label_en, label_se) VALUES
      ('product_name', 'Product / Service', 'Produkt / Tjänst'),
      ('in_price', 'In Price', 'Inköpspris'),
      ('price', 'Price', 'Pris')
    ...
    await pool.query(`
      INSERT INTO pricelist_items (title_en, title_se, in_price, price)
      VALUES
      ('Wireless Mouse', 'Trådlös mus', 10, 20),
      ('Mechanical Keyboard', 'Mekaniskt tangentbord', 35, 60),
      ('Bluetooth Speaker', 'Bluetooth-högtalare', 25, 50),
      ('Smart Watch', 'Smartklocka', 70, 120),
      ('USB-C Charger', 'USB-C-laddare', 8, 15),
      ('Gaming Headset', 'Gaming-headset', 45, 80),
      ('External SSD 1TB', 'Extern SSD 1TB', 60, 100),
      ('Laptop Stand', 'Laptopstativ', 15, 30),
      ('HDMI Cable', 'HDMI-kabel', 5, 10),
      ('Wireless Earbuds', 'Trådlösa öronsnäckor', 40, 70),
      ('Power Bank 10000mAh', 'Powerbank 10000mAh', 20, 35),
      ('Smartphone Case', 'Smartphone-fodral', 5, 12),
      ('LED Desk Lamp', 'LED-skrivbordslampa', 18, 35),
      ('Portable Fan', 'Bärbar fläkt', 10, 18),
      ('USB Flash Drive 64GB', 'USB-minne 64GB', 7, 15),
      ('Wireless Keyboard', 'Trådlöst tangentbord', 25, 45),
      ('Laptop Bag', 'Laptopväska', 20, 40),
      ('Monitor 24-inch', 'Bildskärm 24 tum', 80, 130),
      ('Webcam HD', 'HD-webbkamera', 25, 50),
      ('Wireless Router', 'Trådlös router', 40, 75)
      ON CONFLICT DO NOTHING;
    `);

    await pool.query(`
    CREATE TABLE IF NOT EXISTS terms_information ( 
      id SERIAL PRIMARY KEY, 
      title_en VARCHAR(255), 
      title_se VARCHAR(255), 
      content_en TEXT, 
      content_se TEXT, 
      close_button_text_en VARCHAR(255),
      close_button_text_se VARCHAR(255),
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
    `);

    const title_en = 'Terms';
    const title_se = 'Villkor';
    const close_button_text_en = 'Close and Go Back';
    const close_button_text_se = 'Stäng och gå tillbaka';

    const content_en = `BY clicking Invoice Now, you choose to register according to the information that you have typed in and the text on the registration page and the terms here, and you at the same time accept the terms here.
...
Have a great day!`;
    ...
    const content_se = `GENOM ATT klicka på Fakturera Nu så väljer ni att registrera enligt den information som ni har lagt in och texten på registrerings sidan och villkoren här, och accepterar samtidigt villkoren här.
...
`;
    await pool.query(
      `
    INSERT INTO terms_information 
    (title_en, title_se, content_en, content_se, close_button_text_en, close_button_text_se)
    VALUES ($1, $2, $3, $4, $5, $6)
    ON CONFLICT (id) DO NOTHING;
    `,
      [title_en, title_se, content_en, content_se, close_button_text_en, close_button_text_se]
    );
    res.json({ success: true, message: "Tables created and data seeded." });
  } catch (error) {
    console.error(error);
